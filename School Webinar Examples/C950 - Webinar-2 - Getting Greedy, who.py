...
for i in range(len(myHash.table) + 1):
    print(
        "Movie: {}".format(myHash.search(i + 1))
    )  # 1 to 11 is sent to myHash.search()
...
def greedyAlgorithmMinExpenses(budget):
    ...
    eLabor = 12.00 * (
        math.ceil(cDVDs / 10)
    )  # Labor is $12.00 for every 10 DVDs, $24.00 for 11 DVDs
    eShipping = 0.50 * cDVDs  # Shipping cost is $0.50 per DVD
    eTotal = eDVDs + eLabor + eShipping
    profit = total - eTotal

    print(
        "${:.2f}-Budget, {}-DVDs, ${:.2f}-Expense, ${:.2f}-Profit ==>".format(
            total, cDVDs, eTotal, profit
        )
    )
